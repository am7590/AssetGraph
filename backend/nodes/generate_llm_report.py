...
NODE_REQUIRES = ["current_ticker", "processed_financials", "income_summary"]
...
def generate_llm_report_node(params):
    ...
    async def node(state: dict) -> dict:
        ...
        if missing_keys:
            ...
            # Return update with error message and placeholder report
            return {
                "markdown_report": f"# Report Generation Failed\n\nMissing required inputs: {error_msg}", 
                "errors": current_errors + [f"{node_name}: {error_msg}"]
            }

        # --- Gather Data from State for Prompt Context --- 
        ticker = state["current_ticker"]
        processed_financials = state["processed_financials"] # Already checked it exists
        income_summary = state["income_summary"] # Already checked it exists

        # Further validation/extraction (defensive coding)
        if not isinstance(processed_financials, dict):
            error_msg = "Processed financials data has invalid format (expected dict)."
            logger.error(f"{node_name}: {error_msg}")
            return {
                "markdown_report": f"# Report Generation Failed\n\n{error_msg}",
                "errors": current_errors + [f"{node_name}: {error_msg}"]
...
"""
        ...
        # Return only the updates (report and errors)
        return {
            **report_update,
